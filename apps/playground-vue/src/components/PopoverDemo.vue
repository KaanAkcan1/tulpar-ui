...
// In <script setup>, the imported `vTulparPopover` const is auto-registered as
// `v-tulpar-popover`. The Options-API / global equivalents are:
//   directives: { tulparPopover: vTulparPopover }   // per-component
//   app.directive("tulpar-popover", vTulparPopover)  // global (main.ts)
// The popover's primary idiom is the Ref form: declare a <tulpar-popover id> with
// rich content, then anchor a trigger via `v-tulpar-popover:ref="'id'"`.
import { ref } from "vue";
import { TulparButton } from "@tulpar-ui/vue";
import { vTulparPopover, vTulparTooltip } from "@tulpar-ui/vue";
import "@tulpar-ui/core/popover";
import { Settings2, UserRound, Filter, Bell, HelpCircle, Loader2 } from "lucide-vue-next";

const activity = Array.from({ length: 12 }, (_, i) => i + 1);

const renamed = ref<string | null>(null);
const ext = ref(false);
const asyncLoaded = ref(false);
const loadedText =
...
function onRename(event: Event) {
  ...
  renamed.value = name || "(empty)";
}

function onAsyncOpen(open: boolean) {
  if (!open) {
    // Reset so re-opening shows the loading state again.
    asyncLoaded.value = false;
    return;
  }
  setTimeout(() => {
...
const refCode = `<!-- The Ref form is the primary popover API: declare the surface with its
     rich content, then point a trigger at it by id via the :ref arg. The
     popover is NON-MODAL — no focus trap, no scroll lock, no background inert. -->
...
<button v-tulpar-popover:ref="'pop-profile'">Open profile</button>`;

const inlineCode = `<!-- The inline directive covers only the trivial plain-text case -->
<button v-tulpar-popover="'A quick note rendered on the popover surface.'">
  Quick note
</button>`;

const formCode = `<!-- Focus MOVES into the surface on open (first focusable element), Tab then
     flows OUT naturally (non-modal), Esc / outside-click dismiss and return
     focus to the trigger. -->
<tulpar-popover id="pop-form" label="Rename project">
  <form @submit="onRename">
    <label>Project name
...
<button v-tulpar-popover:ref="'pop-form'">Rename…</button>`;

const toneCode = `<!-- Five tones tint the surface accent. -->
<tulpar-popover id="pop-danger" tone="danger" label="Delete">…</tulpar-popover>
<button v-tulpar-popover:ref="'pop-danger'">Delete project</button>`;

const flipCode = `<!-- placement="auto" (or any side) flips to stay inside the viewport when
...
<button v-tulpar-popover:ref="'pop-edge'" style="margin-left:auto">Near the edge</button>`;

const scrollCode = `<!-- Long content scrolls inside a capped surface (max-height set in CSS on
...
<button v-tulpar-popover:ref="'pop-scroll'">Activity log</button>`;

const asyncCode = `<!-- Load content after open, then call reposition() so the surface re-anchors
     to its new size. A ResizeObserver also re-positions automatically on growth.
     The directive's onOpenChange callback is the open-event hook. -->
<tulpar-popover id="pop-async" label="Details">
  <div>{{ asyncLoaded ? loadedText : 'Loading…' }}</div>
</tulpar-popover>
<button v-tulpar-popover="{ ref: 'pop-async', onOpenChange: onAsyncOpen }">Load details</button>

<script setup>
function onAsyncOpen(open) {
  if (!open) return;
  setTimeout(() => {
    asyncLoaded.value = true;
    document.getElementById('pop-async')?.reposition();
  }, 700);
}
<\/script>`;

const controlledCode = `<!-- Drive open state from elsewhere via the config 'open' field, read it via
     onOpenChange. -->
<button @click="ext = !ext">Toggle from here</button>

<tulpar-popover id="pop-ext">Opened from an outside button.</tulpar-popover>
<button v-tulpar-popover="{ ref: 'pop-ext', open: ext, onOpenChange: (o) => (ext = o) }">
  Anchor
</button>`;

const nestedCode = `<!-- A tooltip can live inside an open popover. The overlay stack closes the
     INNER tooltip first on Esc, then the popover on a second Esc. -->
<tulpar-popover id="pop-nested" label="Settings">
  <button v-tulpar-tooltip="'Controls who can see this project'">Visibility ⓘ</button>
</tulpar-popover>
<button v-tulpar-popover:ref="'pop-nested'">Settings</button>`;
</script>
  ...
  <div class="demo-page">
    ...
    <section class="doc-section">
      <div class="hero">
        ...
        <TulparButton severity="primary" :icon="UserRound" v-tulpar-popover:ref="'pop-hero'">
          Account
        </TulparButton>
      </div>
    </section>
    ...
    <section class="doc-section">
      ...
      <p class="section-desc">
        ...
        point a trigger at it via <code class="inline-code">v-tulpar-popover:ref</code>. You own the
      ...
      <div class="preview">
        ...
        <button class="plain-btn" v-tulpar-popover:ref="'pop-profile'">Open profile</button>
      </div>
    ...
    <section class="doc-section">
      ...
      <p class="section-desc">
        For a trivial note, <code class="inline-code">v-tulpar-popover="text"</code> creates the
        surface from a string. Anything richer should use the Ref form above.
      </p>
      <div class="preview">
        <button
          class="plain-btn"
          v-tulpar-popover="'A quick note rendered on the popover surface.'"
        >
          Quick note
        </button>
      </div>
    ...
    <section class="doc-section">
      ...
      <div class="preview">
        <tulpar-popover id="pop-form" label="Rename project">
          <form class="pop-form" @submit="onRename">
        ...
        <button class="plain-btn" v-tulpar-popover:ref="'pop-form'">Rename…</button>
        <span v-if="renamed" class="state-badge state-badge--on">Saved: {{ renamed }}</span>
      </div>
    ...
    <section class="doc-section">
      ...
      <div class="preview preview--baseline">
        ...
        <button class="plain-btn" v-tulpar-popover:ref="'pop-info'">Info</button>
        ...
        <button class="plain-btn" v-tulpar-popover:ref="'pop-danger'">Delete project</button>
      </div>
    ...
    <section class="doc-section">
      ...
      <div class="preview">
        ...
        <button class="plain-btn edge-trigger" v-tulpar-popover:ref="'pop-edge'">
          <Filter :size="15" />
          Near the edge
        </button>
      </div>
      ...
      <div class="preview">
        <tulpar-popover id="pop-scroll" label="Activity">
          <div class="scroll-body">
            <div v-for="n in activity" :key="n" class="activity-row">
              <span class="activity-dot"></span>
              <span>Event #{{ n }} — user updated the record</span>
            </div>
          </div>
        </tulpar-popover>
        <button class="plain-btn" v-tulpar-popover:ref="'pop-scroll'">
          <Bell :size="15" />
          Activity log
        </button>
      </div>
      ...
      <div class="preview">
        <tulpar-popover id="pop-async" label="Details">
          <div class="async-body">
            <p v-if="asyncLoaded" class="pop-text">{{ loadedText }}</p>
            <span v-else class="async-loading">
    ...
    <section class="doc-section">
      ...
      <div class="preview preview--baseline">
        ...
        <button
          class="plain-btn"
          v-tulpar-popover="{ ref: 'pop-ext', open: ext, onOpenChange: (o) => (ext = o) }"
        >
          Anchor
        </button>
    ...
    <section class="doc-section">
      ...
      <div class="preview">
        <tulpar-popover id="pop-nested" label="Project settings">
          <div class="pop-menu">
            <button class="pop-menu-item" v-tulpar-tooltip="'Controls who can see this project'">
        ...
        <button class="plain-btn" v-tulpar-popover:ref="'pop-nested'">
          <Settings2 :size="15" />
          Settings
        </button>
      </div>
    ...
    <section class="doc-section doc-section--note">
      <p class="note">
        ...
        <strong>Dark mode</strong> recolours the surface via tokens; <strong>reduced motion</strong>
        drops the animation.
      </p>
    </section>
  </div>
