...
const MENU_SNIPPET = `import {
  TulparShellComponent,
  TulparTopbarComponent,
  TulparSidenavComponent,
  type TulparNavItemNgData,
} from '@tulpar-ui/angular';
import { SquareMousePointer, FormInput } from 'lucide-angular';

// Lucide icons are plain data — wrap each in a tiny standalone component so the
// sidenav can project it (the wrapper renders component icons via ngComponentOutlet).
@Component({
...
class IconButton {
  readonly icon = SquareMousePointer;
}
...
readonly menu: TulparNavItemNgData[] = [
  {
    ...
    label: 'Components',
    items: [
      { label: 'Button', href: '/button', icon: IconButton, kbd: 'B' },
      // A nav-item with its own \`items\` becomes a collapsible group (chevron).
      {
        label: 'Form Inputs',
        icon: IconFormInputs,
        items: [
          { label: 'TextInput', href: '/text-input', count: '3' },
...
const SHELL_SNIPPET = `<tulpar-shell-ng
  sidenavMode="static"
  sidenavLayout="under-topbar"
  persistKey="my-app-shell"
  [dark]="dark()"
  [(asideOpen)]="asideOpen"
>
  <tulpar-topbar-ng slot="topbar">…</tulpar-topbar-ng>
...
<tulpar-sidenav-ng slot="sidenav" [items]="menu" [showModeSelection]="false" />`;
...
export class GuideDemoComponent {
  readonly menuSnippet = MENU_SNIPPET;
  readonly shellSnippet = SHELL_SNIPPET;
