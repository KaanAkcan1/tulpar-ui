...
const VARIANTS_CODE = `<!-- variant: text (multi-line bars) · rect · circle -->
...
const LINES_CODE = `<!-- text variant: control the number of line bars; last line is shortened -->
<tulpar-skeleton-ng variant="text" [lines]="1" />
<tulpar-skeleton-ng variant="text" [lines]="3" />
<tulpar-skeleton-ng variant="text" [lines]="5" />`;

const SIZE_CODE = `<!-- explicit width / height / radius overrides (any CSS length) -->
<tulpar-skeleton-ng variant="rect" width="120px" height="32px" />
<tulpar-skeleton-ng variant="rect" width="160px" height="32px" radius="999px" />
<tulpar-skeleton-ng variant="rect" width="100%"  height="120px" radius="12px" />`;

const ANIM_CODE = `<!-- animation: shimmer (default) · pulse · none -->
...
const COMPOSED_CODE = `<!-- Compose skeletons into a card placeholder to reserve layout (no CLS) -->
<div class="card">
...
const SWAP_CODE = `// Real-world pattern: show skeletons while loading, swap to content when ready
@if (loading()) {
  <tulpar-skeleton-ng variant="text" [lines]="3" />
} @else {
  <p>{{ content() }}</p>
...
const PAGE_STYLES = `
  ...
  /* ── Skeleton specifics ───────────────────────────────────────────────── */
  .skel-variants { align-items: flex-start; gap: 24px; }
  .skel-block {
    display: flex;
  ...
  .skel-block--circle { min-width: 0; flex: none; max-width: none; }
  .skel-caption { font-size: 11px; color: var(--tulpar-color-text-muted, #74777a); }
  .skel-card {
    display: flex;
    gap: 12px;
    align-items: center;
  ...
  .skel-card-body { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 8px; }
  .skel-fill { width: 100%; max-width: 360px; }
  ...
  .swap-content { font-size: 14px; color: var(--tulpar-color-text-primary, #15110b); line-height: 1.6; margin: 0; }
      ...
      <div class="preview preview--col">
        <div class="pg-controls">
          ...
          <label class="pg-field">
            ...
            <select class="pg-select" [value]="pgAnim()" (change)="setPgAnim($event)">
            ...
            <div class="swap-card">
              <div class="swap-row">
                ...
                <div style="flex:1; min-width:0; display:flex; flex-direction:column; gap:8px">
                  @if (loading()) {
                    <tulpar-skeleton-ng variant="text" [lines]="2" />
                  } @else {
...
export class SkeletonDemoComponent {
  ...
  readonly linesCode = LINES_CODE;
  readonly sizeCode = SIZE_CODE;
  readonly animCode = ANIM_CODE;
  readonly composedCode = COMPOSED_CODE;
  readonly swapCode = SWAP_CODE;

  readonly variants: SkeletonVariant[] = ['text', 'rect', 'circle'];
  readonly animations: SkeletonAnimation[] = ['shimmer', 'pulse', 'none'];
  ...
  reload(): void {
    ...
    this._timer = setTimeout(() => this.loading.set(false), 1600);
  }

  // ── 7. Live playground state ───────────────────────────────────────────────
  readonly pgVariant = signal<SkeletonVariant>('text');
  readonly pgAnim = signal<SkeletonAnimation>('shimmer');
  readonly pgLines = signal<number>(3);
  readonly pgWidth = signal<string>('');
  readonly pgHeight = signal<string>('72px');

  setPgVariant(e: Event) {
    this.pgVariant.set((e.target as HTMLSelectElement).value as SkeletonVariant);
  }
  setPgAnim(e: Event) {
    this.pgAnim.set((e.target as HTMLSelectElement).value as SkeletonAnimation);
  }
  setPgWidth(e: Event) {
    this.pgWidth.set((e.target as HTMLInputElement).value);
  }
  setPgHeight(e: Event) {
    this.pgHeight.set((e.target as HTMLInputElement).value);
  }

  readonly pgSnippet = computed(() => {
    const v = this.pgVariant();
    const anim = ` animation="${this.pgAnim()}"`;
    const w = this.pgWidth() ? ` width="${this.pgWidth()}"` : '';
    if (v === 'text') {
      return `<tulpar-skeleton-ng variant="text"${anim} [lines]="${this.pgLines()}"${w} />`;
    }
    const h = this.pgHeight() ? ` height="${this.pgHeight()}"` : '';
    return `<tulpar-skeleton-ng variant="${v}"${anim}${w}${h} />`;
  });
}
