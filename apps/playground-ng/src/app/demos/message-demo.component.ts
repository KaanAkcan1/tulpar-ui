...
const TONES_CODE = `const msg = inject(TulparMessageService);
...
const GROUPING_CODE = `const msg = inject(TulparMessageService);
...
const DURATION_CODE = `const msg = inject(TulparMessageService);
...
const WHEN_TO_USE_CODE = `// Message: top-center, single line, 3s auto-dismiss, duplicate grouping.
...
export class MessageDemoComponent {
  protected readonly msg = inject(TulparMessageService);

  readonly groupCount = signal(0);

  // ── Code snippets ────────────────────────────────────────────────────────
  readonly tonesCode = TONES_CODE;
  readonly groupingCode = GROUPING_CODE;
  readonly durationCode = DURATION_CODE;
  ...
  showUngrouped() {
    this.msg.info('Senkronize edildi.', { group: false });
  }
